...
import com.github.finagle.roc.postgresql.Message
import com.github.finagle.roc.postgresql.Startup

trait PostgresqlRichClient { self: com.twitter.finagle.Client[Message, Message] => 
...
object Postgresql extends com.twitter.finagle.Client[Message, Message] with PostgresqlRichClient {

  case class Client(
    stack: Stack[ServiceFactory[Message, Message]] = StackClient.newStack,
    params: Stack.Params = StackClient.defaultParams + DefaultPool.Param(
        low = 0, high = 1, bufferSize = 0,
        idleTime = Duration.Top,
        maxWaiters = Int.MaxValue)
  ) extends StdStackClient[Message, Message, Client] with PostgresqlRichClient {
    protected def copy1(
      stack: Stack[ServiceFactory[Message, Message]] = this.stack,
  ...
  protected def newDispatcher(transport: Transport[Packet, Packet]): Service[Message, Message] = 
    postgresql.ClientDispatcher(transport, Startup(params))
  ...
  def newClient(dest: Name, label: String): ServiceFactory[Message, Message] = 
    client.newClient(dest, label)

  def newService(dest: Name, label: String): Service[Message, Message] =
